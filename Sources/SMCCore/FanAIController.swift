...
//
// 命名声明（诚实化）：这是"目标温度 + 趋势/功耗前馈的自适应预测控制，含在线热
// 参数辨识（ThermalModel RLS / ThermalLearn 查表）"——不是神经网络，不做机器学习
// 训练，不联网。类名 FanAIController 沿用历史（改名将破坏 Codable 兼容与项目惯性），
// 理解为 AdaptiveController 即可。
//
// 与「智能加强(曲线)」的本质区别：
