...
- **做法**（`TestsFamily.swift` 末尾新增 group「双风扇不对称量程扫描(R44)」，`main.swift` 注册）：4 个量程对（本机实测 / 窄量程+宽量程 / 高底噪+可停转 / 等量程）× 6 档百分比阶梯，锁四条不变量——① `percent→RPM` 映射：0%→min、100%→max、阶梯单调不减、永不越出各自量程；② 两把都跟随时**一拍都不许**判闭环故障（假 controlFault = 交还 + 停学习）；③ 其中一把恒停转时**必判**故障（≤8 拍），且两个下标都试——健康那把不得把坏那把顶掉；④ 三条字面量结构门：形状对=4、阶梯=6 档、遍历数=40。
