...
- **首次实证**（run `35962575250`，job success）：干净机器上 `✅ 安装完成！` → 全部断言通过 → `✅ 已完全卸载，风扇已交还 macOS 系统调度` → `首装→运行→卸载 全链在干净机器上通过`。该 runner 无风扇：`fanctld` 扫到 `CPU:1 GPU:0 …`、`FNum=0` 走 4.0 B1 的 passive 语义而不是崩，这本身是硬件泛化性的一次真机（虽非带风扇的机器）验证。
