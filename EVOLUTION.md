...
### R24（4.1.3(71)）：累计改动独立对抗审查——干净视角扫 R21→HEAD，抓到 R23 修复亲手引入的 P1 集成 bug
...
- **P1-1（R23 振荡修复亲手引入的回归，本轮最重要发现）**：试探协议 `probeOK` 分支写 `setForcedRPM`（Md=1 夺权）却**不置 `forcedModeActive=true`**，而"交还 + 退避递增 + 日志"全锁在 `if forcedModeActive` 里。第 1 轮交还把它置 false 后，**第 2 轮起** probe 夺权但标志为 false → 验证失败也不交还/不退避/不日志 → **健康风扇被永久钉在上次试探的强制 RPM**（正是 v2.6.2 注释要消灭的"钉过期 RPM 却谎报已交还"旧洞），退避只走一步（宣称的 30s→32min 实为固定 60s）。根因：R23 删"空拍自解 faulted"时，旧版靠自解→正常重申的**隐性兜底**消失，暴露了 probe 从不置位这个既有缺口。修：probeOK 分支补 `forcedModeActive = true`。
- **P2-1（同修复的第二个组合洞）**：恢复唯一证据 `matched` 沿用检测侧 `target>min+150` 高门槛 → 锁存后系统转凉/夜间、试探只命令低目标 → 候选风扇被 `continue` 跳过、`matched` 恒 false → 已恢复的风扇要等温度爬升才解除（凉机期 faulted 死锁 + 每 60s 钉一次）。修：恢复侧单独判据——低目标命令下"实际≈命令"也算 matched（真停转 actual≈0 偏离 min 命令仍不算）。
- **测试为何没兜住**：R23 场景 11 只断言 `handbacks>=1`（120s 内恰第 1 轮那一次），对"第 2 轮起永不交还"失明。修后加强为 `handbacks>=2`（修复前恒 1、修复后每轮交还），成为 P1-1 的精确回归质。
- **P3 批**：saveConfig 写循环不辨 EINTR（root daemon 有信号活动，write 被打断会静默弃写）→ `n<0 && errno==EINTR` 重试；`sha256Hex` 注释仍写"nil=跳过门"与调用方新 fail-closed 矛盾（会误导未来重开旁路）→ 更正；upgrade.sh marker 的 `-L` 守卫原在全部安装动作之后（同 uid 抢建符号链接 → "系统已升级却报失败、新 App 无人重启"）→ 追加 bootout 前的早期守卫（末尾守卫保留防 TOCTOU 截断）；内嵌占位脚本无同步门 → CI 加"占位==upgrade.sh"防漂移步（build.sh glob 引号经核为误报，前缀已引号且 pathname 展开不词分割）。
- **元经验**：① 删掉一个"错误机制"（空拍自解）前要先问它是否**隐性兜底**了别的洞——R23 的振荡修复把 faulted 生命周期收紧，才让 probe 不置位这个老缺口从"被掩盖"变"暴露"；② 安全/控制类修复的测试要断言**稳态行为**（每轮都交还）而非**首过行为**（第一次交还），否则第 2 轮起的回归全盲；③ 独立视角审累计 diff 的价值 = 作者自审多轮仍漏 P1，"改完必编译"防不住"逻辑组合洞"。
- **验证**：4533 断言全绿（含加强场景 11）、ASan/TSan 双消毒器 0 报告、三目标编译、占位重同步。
