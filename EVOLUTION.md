...
- **验证**：4 条新断言（含控制字符 / 超长 / 非 ASCII 三种输入都出固定标记、常规串照过、**被拒原文一个字节都不留在报告里**、小节数不变）。变异验证：把 `sanitizeVersion(raw) ?? 标记` 退回 `one(raw)` → 三条断言同时红。
...
- **记账**：4860 → **4866 断言 / 85 组**（契约门槛双源同步 4860 / 84）；root 门禁 45/0；`build.sh` 全链路 EXIT 0，`dist/fanctld -v` = `fanctld 4.2.4 (95)`。
