...
- **已知边界（不许被"门全绿"读成已修）**：
  ...
  ② 30 天保留窗按**行**裁（`archiveDay` 的 `count > 30`）⇒ 每多一条空日就挤掉最老的一行；
     极端时 K 条空日把温度窗压到 30−K（K ≥ 17 会让散热趋势卡退回"积累中"）。空日占位是有意的
     （那天的磨损确实发生），但"窗按行不按有效日"记在这里。
...
## 交接口（2026-09-26 · 仓库/发行 = 4.2.18(109)，main=origin/main，工作区干净）
...
- **R52 的观感待答一条仍挂着**：6 处"每拍抖动的量"撤掉了转场/补间（主温度、GPU/部件温度、两条条形、功耗胶囊），
  两颗胶囊改换档才演——需要作者开一眼确认没把要的设计改坏；R53/R54/R55 **没有**再改任何观感
  （R55 只动了入账门与天数口径，面板可见性一条明确记成边界）。
