...
## 交接口（2026-09-26 · 仓库/发行 = 4.2.16(107)，main=origin/main，工作区干净）
- **本机跑的不是最新版（这是最大的那条）**：装在系统里的 daemon 早于 4.2.8，所以 R43（失联风扇交还）、
  R45（磨损口径）、R46（安全托底即时落盘）、R47/R49（转速/占比等宽直读）、R51/R52（面板动画收口）、
  R53（第二个分母）在真机上**都还没生效**。要生效必须作者亲自 `sudo ./scripts/install.sh`（我不跑 sudo，属自治边界）。
...
- **被数据卡住的一条**：控制律 / AI / PD 参数不动，dt 账本 3.67 天 < 宪法要求的 7 天；别绕，也别拿 VM 仿真当真机标定。
