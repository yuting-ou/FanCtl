...
- **改前→改后**（复现：跑 `fanctltests`，group「零可用温度日不吞账(R55)」）：
  ...
  - 断言 5016 → **5033 / 92 组**；门槛双源 5005 → **5025**；VERSION `4.2.17(108)` → **`4.2.18(109)`**。
- **变异证据**（同一会话真跑、逐文件 `cmp` 复原、复绿 5033/92）：
...
- **R52 的观感待答一条仍挂着**：6 处"每拍抖动的量"撤掉了转场/补间（主温度、GPU/部件温度、两条条形、功耗胶囊），
  两颗胶囊改换档才演——需要作者开一眼确认没把要的设计改坏；本轮（R53）**没有**新改任何观感。
- **被数据卡住的一条**：控制律 / AI / PD 参数不动，dt 账本 3.68 天 < 宪法要求的 7 天；别绕，也别拿 VM 仿真当真机标定。
...
- **下一轮开工第一件事（性能，不是观感）**：面板开着时约 2/3 的拍要 ~800ms、原因未定。先立复测协议再动手（R52 末尾），并补一次上屏态重复基线。
