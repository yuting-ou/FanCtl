...
## 交接口（2026-09-25 · 仓库/发行 = 4.2.15(106)，main=origin/main，工作区干净）
- **本机跑的不是最新版**：装在系统里的 daemon 早于 4.2.8；4.2.8/4.2.10–4.2.14 只在仓库与 Release 里。要生效必须作者执行 `sudo ./scripts/install.sh`（我不跑 sudo，属自治边界）。
- **只有作者能答的三条**：① 装最新版；② App 内一键升级的 root 脚本自我刷新，需要一台装着旧版的机器；③ 空 tag `v4.1.4` 的处置（删不删归作者）。
- **R50 的观感待答一条已用数据自答**：两处动画**保持原样**（撤掉不省成本），不再需要作者裁定；面板开一眼只需确认观感没被我改坏（R47/R49 改的转速与功耗占比仍是"等宽直读"）。
- **被数据卡住的一条**：控制律 / AI / PD 参数不动，dt 账本 < 宪法要求的 7 天；别绕，也别拿 VM 仿真当真机标定。
...
- **本轮已欠一次上屏态复测**：面板每拍成本的绝对值只在离屏态确立（见 R51 残余①），下一轮开工先跑重复基线再动手。
