...
- **交付状态（诚实）**：4.2.24(115) 已提号、已过 build.sh 全链路与 5111 断言、已推 main；
  **tag 未打、Release 未发**——剩余轮次不够走完"tag → CI → 资产解包验证"，按宪法不许交一个
  没验过的 Release。下轮第一件事就是 `git tag -a v4.2.24 && git push origin v4.2.24` 并解包验证。

## 交接口（2026-09-26 · 仓库 = 4.2.24(115)（**tag 未打**），GitHub Release = 4.2.23(114)，main=origin/main，工作区干净）
...
- **本机跑的不是最新版（这是最大的那条）**：装在系统里的 daemon 早于 4.2.8，所以 R43（失联风扇交还）、
  R45（磨损速率口径）、R46（安全托底即时落盘）、R47/R49（转速/占比等宽直读）、R51/R52（面板动画收口）、
  R53/R54（磨损速率的第二个分母与 14 日趋势的墙钟覆盖率）、R55（零可用温度日不再吞账）、R56/R57（归档窗按日历日裁＋坏日键不配当一天）、R60/R61（量具逐拍序列、comboBar 每拍数字不再滚）、
  R56（保留窗按日历日裁）、R57（坏日键/未来日不冒充数据日）在真机上**都还没生效**。要生效必须作者亲自
  `sudo ./scripts/install.sh`（我不跑 sudo，属自治边界）。装完顺手跑一次 `fanprobe`：趋势表右列的
  **墙钟覆盖率**（本机 68% → 8~19%）就是这两轮的结论本身——左列那份"次/采样分"翻倍是分母在塌，
  不是风扇在恶化。
