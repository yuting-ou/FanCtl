...
- **量出的真问题（下一轮头号）**：面板开着时**每一拍数据要数百毫秒 CPU**（不接窗口只要 10.6ms，
  ...
  **不是无界泄漏**——只看前 20 拍会误判成 6–8MB/拍的泄漏，这条口径记下来。
  ⚠️ 未定论项：`--onscreen`（真上屏）两次分别 295 / 826 ms/拍，与离屏 804–834 之间**没有稳定关系**，
  所以"数百毫秒"这个**绝对值**外部效度未确立（差值的结论不受影响：同一形态下 A/B 对照仍成立）。
...
- **记账**：VERSION `4.2.13(104)` → **`4.2.14(105)`**；断言 **4947 → 4968 / 89 → 90 组**；
  契约门槛双源 4930 → **4950**（`ci.yml` 与 `fanctltests/main.swift` 同值）。
- **残余风险（不许当已修）**：① 每拍数百毫秒的**绝对值**未在上屏态确立，且**没修**——本轮只造量具
