...
- **已知边界（不许被"门全绿"读成已修）**：
  ...
  ② 未来日与坏键行不被日历裁删（只受 60 行封顶）⇒ 读侧 `dataDayCount`、`after(baselineDate:)`、
     `suffix(14)` 仍会把它们当"一行"，未来日能让"AI 启用 N 天"虚高一天。读侧一刀切属"改日键口径"，
     与 R53 边界①同族，归作者决策。
...
## 交接口（2026-09-26 · 仓库/发行 = 4.2.19(110)，main=origin/main，工作区干净）
- **本机跑的不是最新版（这是最大的那条）**：装在系统里的 daemon 早于 4.2.8，所以 R43（失联风扇交还）、
  ...
  R56（保留窗按日历日裁）在真机上**都还没生效**。要生效必须作者亲自
