...
- **改了什么**：`Sources/FanCtlApp/TickBench.swift:160-163` 新命名常量 `let rssSampleEvery = 1`，
  `:186` 采样条件改 `i % rssSampleEvery == 0`；`:211` 出口新增 `rss_points=\(rssSeries.count)`。
  门（`Sources/fanctltests/TestsCore.swift:1438-1446`，group「量具逐拍序列(R60)」扩到 R62）加 4 条：
...
## 交接口（2026-09-26 · 仓库/发行 = 4.2.23(114)，main=origin/main，工作区干净）
