...
## 交接口（2026-09-25 · 仓库/发行 = 4.2.14(105)，main=origin/main，工作区干净）
...
| R51(观感/性能) | 撤掉两处隐式动画（`FanModel:722` 事务级 `withAnimation`、`GaugeViews:154` 卡级 `.animation(value: temp)`）能省下面板每拍的 CPU | 每拍成本显著下降 → 值得用观感去换 | **不降**：留着 804/818/831/834ms/拍 vs 撤掉 848/867ms/拍（差在噪声内、方向还反了）。同样不降的还有"玻璃换实心材质"(821/815) 与"主温度去渐变/辉光/转场"(852/854)。真正的成本在 `CA::Transaction::flush → 整树 display → 字形从轮廓重栅格化`，与补间无关 | 把"动画 = 持续 60fps 重绘"这个直觉套在了**一拍一次、0.45s 内收敛**的补间上；成本主体是发布面太大（一个 `@Published` 标脏全部文字层），不是补间时长 | **不要再为了省 CPU 去撤这两处动画**（也不要以"省 CPU"为理由撤任何单点 `.animation(value:)`）。要降面板成本就去**拆发布面**，并且必须先有 `--tickbench` 的前后数字，不许凭直觉下刀 |
