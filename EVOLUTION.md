...
## R64（4.2.25(116)）：量出来的"风扇图标自己缩小"——旋转层的 frame 是外接框，赋 frame 会腐蚀 bounds
...
- **顺带记一条**：`--plain`（快照降级）走的是 SF Symbol 静态图标，两行必然一致 ⇒ 它**不能**用来判这条缺陷，
  别把"plain 下正常"当成"没问题"。
- **执行结果（先量后改，量具直接指出成因）**：`TickBench` 加 `fan_geom=` 出口，打印每个 spinner 的
  `frame/bounds/layer/sublayer(frame+bounds+position)`。改前实测两个图标层：
  `#0:29x33+img@12,12` 与 `#0:27x33` / 另一次 `29x21` 与 `31x18` —— **视图本身 f/b/l 全是 24×24，
  坏在 iconLayer 的 frame 是非方形**。⇒ 成因不是"SwiftUI 没给尺寸"，而是
  **旋转中的 CALayer 读 `frame` 得到的是旋转后的外接框**，于是 `setRPM` 里那句
  `if iconLayer.frame != bounds { iconLayer.frame = bounds }` 恒真，每拍给 `anchorPoint=(0.5,0.5)`
  的旋转层赋 frame，而赋 frame 会反算 `bounds`/`position` ⇒ bounds 被逐拍腐蚀，图标一格一格缩小。
  **修法**：`GaugeViews.swift:316-321 / :347-351` 两处都改成只写 `bounds` + `position`（与 transform 无关、幂等）。
  改后实测：`bd24x24`（两行、多次运行全部一致），`fr28x28`/`fr33x33` 只是旋转外接框的正常读数；
  截图复核两行图标同尺寸同形状。门加两条（`iconLayer.frame =` 不得再现、`iconLayer.bounds = bounds` 恰好 2 处）。
- **R64 的已知边界**：① 低占比那条"能量条退化成孤立小圆点"**没修** —— 它是 `max(4, w×占比)` 的
  刻意下限（转速贴着下限时确实只剩一小截），要改属观感决策（要不要给可见最小长度 / 是否加"贴着下限"标记），
  留给作者一句话；② 本机的风扇图标只在 RPM>50 时旋转，`stopped` 分支的几何未单独验；
  ③ 量具的 `fan_geom=` 只在 `--tickbench` 里，普通运行不产出（诊断入口，不进发行路径）。
- **交付**：VERSION `4.2.24(115)` → **`4.2.25(116)`**；断言 5111 → **5114 / 95 组**；门槛双源 5105 → **5110**。
  **发行链闭合**：tag `v4.2.25` → CI success → 徽章回推 fast-forward 合并 → Release 资产解包已验
  （6 项 / bundle 内 0 脚本 / `install.sh` 755 / `fanctld 4.2.25 (116)`）。
  **本机 App 已热替换到 4.2.25**（`deploy.sh` 免密），装机实测 `fan_geom` 两个 spinner 均
  `bd24x24`（改前是非方形外接框）⇒ 这条观感缺陷在真机上也确认修好了。
  真机 daemon 仍是 `4.2.24 (115)`——4.2.25 的 diff 只碰 App/测试/文档，daemon 侧除版本串外**无代码差异**，
  因此没有为它单独再弹一次 root 授权；下次有真正的 daemon 改动时一并升。

## 交接口（2026-09-26 · 仓库/发行 = 4.2.25(116)，main=origin/main，工作区干净）
