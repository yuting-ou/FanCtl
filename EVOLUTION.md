...
- **下一轮最高杠杆候选**：①②（把缺口做成 status 的显式字段并进 `statusChangeSummary`，让 App/fanprobe 有同源载体）——但要先确认这不算作者已钉死的 UI 范围；控制律/AI 参数仍按宪法延后（dt 账本 3.4 天 < 7 天）；带风扇真机的热闭环、GUI 起窗、App 内一键升级三条仍只有作者能答。
