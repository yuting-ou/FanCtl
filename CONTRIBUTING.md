...
7. **改动 SMCCore 后必须跑 fanctltests**：测试与 daemon 共用同一份决策代码，防止镜像漂移。改动 `scripts/upgrade.sh`/`install.sh` 同样必须跑（会自动调 root 脚本门禁）。4.2.0 起 root 执行脚本不再有内嵌占位（`install.sh` 装进 `/usr/local/libexec`、`upgrade.sh` 每次升级自我刷新），改完只需跑门禁。
