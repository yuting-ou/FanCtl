...
- **写入闭环可观测**:`WriteHealth`(连续写失败 5 拍 fault)+ `FanFeedbackHealth`(实际 RPM 不跟随目标;升速宽限+故障锁存——交还/匹配都计恢复进度，自解路径永在故结构上不可能永久锁存；首次故障 3 拍解除，反复故障按 3→6→12→24→48 拍指数退避，确认真实跟随即归零)。故障期间每 30s 试探接管一次(写-验证-交还协议,严格 3 拍验证窗)。
