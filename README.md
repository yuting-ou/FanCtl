...
- **可验证**:数千项纯逻辑断言（含 81 成员参数化 HIL 闭环扫描），CI 每次推送自动回归——准确数字见顶部 tests 徽章
- **可解释性**:status.json 携带"当前转速由谁决定"(reason)与 AI 实时意图,UI 直接展示
- **一键升级**:查到新版本后菜单点一次,App 内完成 下载→校验→管理员授权安装→重启(全程一次系统授权弹窗,无需终端)
...
| `config.last-good.json` | 4.1.4：每次成功解码后滚动的"最后一份**消毒过**能用的配置"（单槽）——损坏时优先回它而非出厂默认 |
| `status.json` | daemon 每拍写出:全部传感器、appliedPercent(s)、fans、reason、aiIntent、learning 状态、功耗、环境温度、故障标志 |
| `stats.json` / `history.json` | 今日战报(温度直方图/功耗累计,跨天归档,保留最近 30 个日历日窗口·4.2.19 起) |
| `ai-learn.json` | 热经验查表(2°C 桶 × 场景桶) |
| `thermal-model.json` | 散热参数线性模型(a=热阻,b=风量效率) |
| `ai-metrics.json` | AI 控制质量评测(均温/波动/均输出——4.1.4 起按秒加权,自适应拍长下不再偏袒繁忙时段) |
...
- **退出/睡眠**:恢复系统自动调度(SIGTERM/睡眠回调);**启动时无条件清理 SMC 强制模式残留**(崩溃/断电重启后风扇不被钉死);**唤醒时闭环健康检测器随引擎状态一并复位**(睡前的故障锁存与命令/RPM 基线不带进新会话,4.1.4)。
- **传感器故障**:连续 5 拍读失败 → 交还系统并写 controlFault 到 status;启动即故障也会写最小状态。
- **写入闭环可观测**:`WriteHealth`(连续写失败 5 拍 fault)+ `FanFeedbackHealth`(实际 RPM 不跟随目标;升速宽限+起转宽限——RPM 相对上拍上升 >50 视为物理爬升，不判停转、滞后也计响应（试探窗内同样成立）+故障锁存——交还/匹配都计恢复进度，自解路径永在故结构上不可能永久锁存；首次故障 3 拍解除，反复故障按 3→6→12→24→48 拍指数退避，全部高目标风扇均确认跟随才归零)。故障期间每 30s 试探接管一次(写-验证-交还协议,严格 3 拍验证窗)。
- **数值防御**:全链路 NaN/Inf 防护(SMC 读数、曲线插值、配置 sanitize、App 端 Int 转换);SMC 键值钳位防 Int()/UInt16() trap。
- **配置损坏自愈**:loadConfig 失败先备份坏文件,再按 **last-good → 默认** 的优先级恢复(4.1.4:一次截断不再把用户曲线/模式抹回出厂);`history.json` 同轮改为**逐日抢救**——整表解码失败时按元素单独解码,坏 1 天丢 1 天而非 30 天归档全灭。
- **落盘失败不静默**:learn/model/dt-ledger 的 dirty 旗只在写成功后清除,失败则保留并在下个节流周期重试;stats/metrics 持续失败打一条边沿日志(静默降级可以,静默失效不可以)。
- **启动 SMC 退避**:开机早期 IOKit 未就绪时进程内重试 3 次(0/2/8s)再交回 launchd——把 KeepAlive+Throttle 下"每 10s 全量扫描→退出"的盲循环压成"每 ~20s 一轮、每轮 4 行日志",并让原因可查(留档 `exit-reason.flag`,下次启动日志直接答"上次为何退出")。**持续失败仍会循环**(退避只是跨过瞬时竞态,不替代根因修复)。
...
# 卸载（4.2.0 起脚本装在 root 拥有路径；旧版路径为 bundle 内 Resources）
sudo /usr/local/libexec/fanctl-uninstall.sh
...
2. **改 SMCCore 后必须跑 `fanctltests`**:测试与 daemon 共用同一份决策代码,覆盖管线优先级、控制律、学习、AI 状态机、Codable 兼容（断言数以顶部 tests 徽章为准）。
...
- **特权信任根（批次 A）**：App 只 exec 经校验的 root 路径脚本——exec 前 `lstat` 判定
  ...
  （Homebrew 常见）则**拒绝安装**，而不是把 root 代码放进可写目录。内嵌 base64 机制整体
  作废（二进制与被删的生成段、CI 漂移门、bundle 内两份脚本副本一起清）。
- **睡醒不再误报**：`wake()` 复位 `WriteHealth`/`FanFeedbackHealth`（睡前的故障锁存与
  命令/RPM 基线不带进新会话）；故障退避 `faultStreak` 跨唤醒保留（那是判决不是测量）。
- **数据诚实**：`history.json` 改逐日抢救（坏 1 天丢 1 天，不再一次局部损坏抹掉 30 天归档）；
  AI 评测的均温/波动/均输出改**按秒加权**（自适应 1–20s 拍下不再偏袒繁忙时段；分母是加权
  覆盖秒，跨版本混合账本不再被旧秒数稀释）；落盘失败保留脏旗下个周期重试 + 边沿日志。
- **自愈退化**：新增 `config.last-good.json`（存"解码成功且已消毒"的副本），配置损坏时按
  last-good → 默认恢复；SMC 启动瞬时失败改进程内退避重试（0/2/8s）并留档 `exit-reason.flag`。
- **发版链自证**：`build.sh` 产物目录一律问 SwiftPM（且与构建同一组 flags、按 `--product`
  请求——runner 那版 `--target` 只编译不链接，本机却会链接，连红四次才看清）；组装后断言
...
- **文本消毒（4.2.4）**：诊断包里凡从盘上读来的自由文本都过一道消毒——身份类（App 版本、daemon
...
- **守卫的守卫（4.2.7）**：上一轮的两个守卫经独立审查补强——卸载的删除前缀不再被 `/.`、`//`、
  单层 `/Users` 这类"非 / 的 / 写法"骗过（先 `cd -P` 归一化再判层数），App 落点谓词先剥尾斜杠再判
  （`test -L path/` 会跟随链接）；四道门禁自身的假绿形态也补了（注释掉的预检、被 `if false` 掏空的
  复验、要求行尾的尾斜杠扫描、查不到就静默跳过的 git mode 检查）。
...
- **失联风扇交还（4.2.8）**：多风扇机器上某把风扇的 `Ac/Mn/Mx/Tg` 读失败（固件 NaN、
  键异常）时，`allStates()` 会跳过它，而写循环只遍历可读风扇——被跳过的那把就此钉在最后一次
  强制目标上（`Md=1` + 过期 `Tg`），系统调度对它失效且无人复查：`writeHealth` 只看写出去的那几把
  （全成功）、`feedbackHealth` 只看可读名单（它不在里面），`status.fans` 只列读得到的一把，
  双风扇机器在 UI/诊断包里长得跟单风扇机器一样，直到 92°C 兜底才可能被整体救回。现在**连续失联
  满 6s** 即按边沿交还系统调度（每轮失联一次 `Md=0` + 一条日志；恢复可见当拍重新接管、再失联
  再交还；唤醒后仍失联会补交还；交还写失败也出声且不逐拍重试）。去抖是必需的：间歇 NaN 的键
  在可见拍上照常受控，单拍即交还会把模式位抖成 v2.6.2 修过的"交还↔夺回"振荡。诊断包"风扇"
  小节只报现象（`另有 N 把本拍无读数`）——处置结论的判据在引擎那一拍，status 快照没资格下。
  健康机器（含单风扇）零变化；单风扇机器上唯一风扇持续读不到时，交还比原来的"5 拍写失败"门更早，方向同向。
- **面板渲染成本量具（4.2.14，`FanCtl --tickbench`）**：本会话开不出真面板（OS 27 不呈现合成
  点击），于是给 App 装了一台离屏量具——真窗口托管真面板、真机 status.json 当夹具逐拍抖动、
  `getrusage`/`mach_task_basic_info` 量每拍 CPU 与内存斜率，并自带三个对照（不接窗口的
  非渲染基线、不喂数据的常驻循环检查、玻璃换实心的归因探针）。它判掉了"撤动画省 CPU"
  （两处动画撤掉后成本不动，故观感保持原样），也量出了真正的大头：**面板开着时每拍数百毫秒
  CPU**，火焰图指向 CA 事务刷新里的字形逐帧重栅格化。写盘只落临时目录，由「渲染量具安全门
  (R51)」静态钉住。
- **磨损速率给了第二个分母（4.2.16）**：`次/采样分` 的分子在温度失真拍也计数、分母不算那些拍
  ⇒ 系统性偏高（R45 记过账，当时只改标签）。现在诊断包与 `fanprobe` 并排印出当日墙钟口径，
  本机实测 17.04 vs 6.79（2.51 倍），并写明两个口径各偏一边、真值落在两者之间；拿不到墙钟
  （归档日/当日不足 30 秒）印 `—` 不印 0。
- **14 日趋势给出第二个读数（4.2.17）**：`fanprobe` 趋势表右列改成**墙钟覆盖率**（当日采样秒 ÷ 86400），
  `12.78 · 8%` 这种写法直接回答"左边那份 `次/采样分` 能信几成"。本机两周覆盖率 68% → 8~19%，同期采样口径
  从 3.21 涨到 16.42——上涨是分母在塌，不是风扇在恶化。顺带把 R53 那句"偏高"的量级校准了：用功耗直方图
  分解，覆盖率下降里约 92–100% 来自整机睡眠/daemon 没跑，只有 0–11% 来自温度门拒收。分母不足 30 秒的日子
  不再整行消失（印 `—` 并给出原因）；趋势行文本搬成有断言的纯函数（此前全仓没有一条测试读过 fanprobe 的
  stdout）。另新增"断言数契约门槛两源同值"的 root 脚本门（此前漏改 CI 下限是静默的）。
- **零可用温度日不再吞账（4.2.18）**：归档与启动恢复的门从 `tempCount > 0` 换成"这天有没有被计过的量"
  （`hasAccountedActivity`）。此前温度整天被拒收时，当天的调速次数/转数/功耗秒一起蒸发——真实磨损掉进
  无记录的黑洞。入账后趋势表把它显式印成 `— · 0%`，温度类读者各自过滤、结论不变（有断言与变异证据）。
- **归档窗口按日历日裁（4.2.19）**：4.2.18 之后 `history.json` 里可能出现"温度整天不可用"的空行，而窗口
  旧裁法是"最多 30 行" ⇒ 空行挤掉真有温度的日子，名义"保留 30 天"实得不足 30 天。现在窗口是
  "与最晚归档日的日历日差 ≤30"（含首末共 31 天；参照点排除未来日、也不取字典序末行，坏日键与未来日
  只占行数封顶的位、且封顶时先让它们走）。数字来自单测夹具（40 个日历日、其中 10 天空日）：
  窗口内 31 行 / 24 个真数据日，旧法是 30 行 / 23 个。
- **不可用日不冒充数据日（4.2.20）**：4.2.19 之后坏日键与「晚于今天」的行会**长期留在** `history.json`
  （日历裁不删它们，只在封顶时优先出局），而「AI 启用 N 天」只数 `tempCount > 0` ⇒ 时钟回拨或手改 JSON
  留下的一行"未来日"照样被数成一天。现在这类行既不计入天数，也在 `fanprobe` 趋势表的行尾**显式标注**
  （`日键不可解析` / `晚于今天（时钟跳变/手改）`）——静默占位才是最难查的那种谎。
- **量具加逐拍序列（4.2.21，仅诊断入口）**：`--tickbench` 现在吐 `tick_ms_series`（每 2 拍一个毫秒值）。
...
- 测试 **4646 → 5102 断言 / 95 组**（断言数与契约门槛以顶部徽章 + `ci.yml`/`fanctltests` 双源为准；
...
4.0 前三批:B1 无风扇(passive)机器语义化 auto + App 诚实提示;B2 冷启动校准期——用户选 AI
但学习表未成熟时先观察系统自控的实际平衡写进学习表(≥2 采信桶 / 12 稳态样本 / 45 分钟超时
后接管,零新增持久化);B3 研究项抓到学习查表单侧外推的欠冷却播种缺陷 → 采信域
lookupBandWidth=10°(出带返回 nil 退回曲线/公式种子)。随后全量对抗审查修复(R16),
断言 4430→**4469**:

- **P2 校准超时假退出**:原实现超时置 calibrating=false,但门控同条件下一拍拉回、窗口重置
  ——45 分钟超时接管实为 45 分钟重试;修 = calibrationTimedOut latch + 超时检查移出风扇
  有效性守卫,超时成为真退出(切离 AI 再切回可重新校准)。
- **P2/P3 FNum 自愈**:fanCount 原为 init 一次性 let,启动瞬时读取失败把有扇机器永久钉在
  无风扇语义(B1 后从响亮 controlFault 变成安静的错误语义);修 = fanCount==0 期间 30s 节流
  重探,恢复即翻正 + 硬件画像同步更正。
- **衍生**:calibrating 进 statusChangeSummary 变化感知(否则校准状态翻转要等 10s 心跳落盘,
  App 提示与测试断言都被陈旧 status 骗)。
- **交付**:4.0.0(60) 为 4.0 首个送达真实机器的版本载体(B1-B3 曾长期只存在于 main,
  本机停留在 3.9.1——R12 同号不同码教训的修复轮)。4.0.1(61) 追加(4.1-A):dt 账本
  独立持久化 dt-ledger.json(生命周期=控制律版本,目标档位切换不再清零);P 校准线
  退役 + 斜率归一化 I_D 口径(预注册裁决规则两处证伪的修复,EVOLUTION R17)。

## 10. v3.9.1 变更摘要(2026-09,对抗循环:一键升级修复与动态验证补全)

v3.9.0 发布后的三轮对抗审查循环 + 动态方法论补全,全部落在升级模块。
断言 4409→**4420**:

- **P1 暂存目录错位(Round A)**:App 传给特权脚本的是解压根目录,而 zip 真实
  布局是 `FanCtl-{版本}/` 子目录 → v3.9.0 的 App 内自动升级首次使用必然失败
  (脚本 exit 2"暂存包不完整")。dogfood 手造 stage 恰好掩盖——**端到端验证
  必须复现真实输入形态**。同轮修:osascript 等密码期间主线程阻塞(P2)、
  安装成功但进程存活的边缘路径菜单死锁(P2)、升级中文案版本与实际下载脱钩
  等 5×P3。
- **重启重设计(Round B)**:root 上下文 open 实测静默失败、asuser submit 的
  进程域归属存疑(菜单栏 App 可能 root 运行,不可接受)→ **root 不做 GUI 拉起**。
  改"遗言 watcher":osascript 前以用户态拉轮询进程,脚本装完只 touch 完成标记,
  App 被 pkill 后 watcher 被 launchd 收养,以登录用户身份 open(与手动打开同路)。
- **动态验证(R11)**:模糊性质测试 500 轮确定性扫描(输出白名单/URL 契约/文案
  无元字符/幂等——首轮抓到的是测试自身的 rangeOfCharacter inverted 误用);
  变异测试 7 变异体全杀(净得分 100%,零盲区);运行态对账(发行包/内嵌脚本/
  daemon 心跳)。
- **遗留清点(R12)**:v3.9.0 Release 资产含上述 P1 且修复无版本载体(本机与
  Release 同号不同码)→ 本版 v3.9.1(59) 作为修复的正式载体,超代损坏资产。

## 11. v3.9 变更摘要(2026-09,一键升级:App 内完成更新闭环)

v3.6 版本自检止步于"提示 + 打开下载页";本轮把安装闭环进 App——菜单点一次
"⬆️ 升级到 vX.Y.Z…",用户只面对一次 macOS 管理员授权弹窗。断言 4368→**4409**:

- **流程**:下载 Release zip(URLSession,资产名契约 = ci.yml release job)→ /tmp 暂存解压
  → 校验门 → osascript 管理员授权执行内嵌 upgrade.sh(装 daemon + bootstrap + App 归还
  登录用户 + 清 quarantine + 自动重启)→ 新版本顶替运行。失败显示原因并给"重试";取消
  授权静默回退;"打开下载页"与"跳过此版本"保留。
- **特权面三层收窄**:①tag 消毒(ASCII 数字+点、≤4 段、≤24 字符)是 URL/shell/授权文案
  三处共用的唯一收窄点,16+ 注入对抗样本断言锁定;②root 执行的 upgrade.sh 内嵌 App
  Resources 与 App 同源发布,绝不从网上下载脚本;③校验门(Info.plist 版本与 Release tag
  **严格相等**——更新版本也不放行 + fanctld 在场)在授权弹窗之前执行,root 动手前最后一道闸。
- **upgrade.sh 与 install.sh 的差异**:osascript 场景无 SUDO_USER——用
  `stat -f%Su /dev/console` 取登录用户归还 App 属主(保住免密 deploy.sh 通道,v3.8 部署
  教训);`launchctl asuser` 以登录用户身份重启 App;`xattr -dr com.apple.quarantine`
  防御性清理(未公证 bundle + quarantine = Gatekeeper 拦截)。
- **测试抓出的消毒漏洞(诚实记录)**:①`Character.isNumber` 对非 ASCII 数字(Nd 类,
  如 ٣)返回 true,消毒器初版放行 `٣.٩`——URL 消毒必须 `isASCII && isNumber`,新测试
  当轮抓到;②struct 无构造器模式匹配(`if case let Failure(msg)` 只适用 enum)。
- 真机 dogfood:v3.8.0 部署已用同型流程手工预演(dist→/tmp + 一次授权弹窗),
  v3.9.0 起走正式内嵌脚本。

## 12. v3.8 变更摘要(2026-09,兑现轮:D 项 dt 账本 + 硬件画像 + 存活去抖)

审计轮收敛后的第一轮"兑现"——让 v3.7 造出的仪表产出可裁决的证据,并为陌生机器铺诊断地基。断言 4323→**4368**:

- **D 项 dt 账本(预注册裁决,R4 跳过项 1 的兑现前置)**:ai-metrics.json 新增按拍长分桶的
  |P|/|D| 生效增量累计(快拍<1.5s/标称 1.5–4.5s/长拍>4.5s)。AIController 每拍转述实际生效
  增量(anti-windup 跳过的 P 记 0;播种/空闲拍不入账),与评测样本同拍对齐入桶。
  **预注册裁决规则(先于数据写下,防事后合理化)**:D 快拍/标称比理论值 3(1/dtNom 漂移);
  账本积累 ≥7 天且快拍占比 >5% 后,若比值≈3 且绝对影响可忽略则关闭议题,若显示真机快拍
  长期高频出现才动公式——改控制律必须有真机账本支撑(策略元经验)。P 基线跨桶应相等,
  不等 = 账本自身有 bug(内置校准线)。fanprobe 直接打印比值。
- **真机对账(P4 兑现检查,2026-09-08)**:高温桶 75°/77°/79°/81° 全部在积累(340/288/71/19 样本),
  学习门不饿死;学习地图 14 点连续单调,包络健康度 5.8→**1.43**(高温段旧非单调数据正在被
  新样本洗净,观察协议详见 EVOLUTION)。
- **硬件画像(N=1 通用化第一块砖)**:机型/芯片/OS/风扇数/传感器计数/功耗键存在性,daemon
  启动采集一次随 status 下发 + fanprobe 打印。画像只描述机器,不含学习数据(机器特异,
  移植即污染)。陌生机器 issue 的"这台机器长什么样"一问即答。
- **存活判定去抖(R4 跳过项 2 加固)**:rename 竞态等单次读失败/status 瞬时陈旧不再让
  daemonAlive 单拍翻转(UI 状态点闪烁+决策行清空回填);下线需连续 2 次死观测(12s 轮询下
  额外 ≈12s,假"活"代价只是多显示一轮旧数据,假"死"代价是清空用户正看着的状态),上线
  立即生效。逻辑抽为 SMCCore `AliveDebouncer` 可单测。
- **防御惯例全程**:新 Codable 字段全部 decodeIfPresent+默认值+读出侧钳位;垃圾模糊池
  覆盖 dt 账本桶与硬件画像(含类型错配路径);画像字段损坏只降级该字段不拖垮 status 解码。
- **测试事故(诚实记录)**:新测试用 `try!`/`!` 强解包导致 2 次 exit133 崩溃——断言前先
  guard 绑定,让失败走断言消息而非 trap;分桶完备性契约从"sampleCount−1"改为
  "Σ桶样本=总样本"(首拍 actualInterval=0 被 record 守卫跳过,算术假设不成立)。

## 13. v3.7 变更摘要(2026-09,信任三角:学习地图+冻结曲线+决策透镜)

计划三遍自审后执行的"看得见"专项(数据流攻击/可用性攻击/结构攻击,14 处修正)——学习系统从黑箱变为可看、可懂、可退出:

- **P3 学习地图**:AI 卡内可折叠图表,采信桶温度→风量折线,点径=置信度;daemon 仅在样本数变化时重算下发(不打穿写盘节流);展开时卡片平滑长高。
- **P1 冻结为曲线**:一键把 AI 稳态经验转成普通曲线(快照语义,不承诺效果;边界锚点继承用户曲线;≥2 采信点才可冻结)。对 AI 不放心时的退路=它学到的形状。
- **P2 决策透镜**:一行"为什么是这个转速"(目标/温度/误差/学习值/迟滞保持/循环抑制),悬停看完整解释;与评测磁贴互斥。
- **计划外 F9(v3.6.0 起的静默失效)**:DaemonStatus 主 init 漏 learnEnvelopeGap 成员赋值——Optional 默认 nil 静默吞掉参数,包络健康度在 status.json 从未出现(fanprobe/App 仪表整版失效)。Optional 字段也必须显式赋值。
- 冻结后的曲线仍受全部安全兜底(92°/SSD/电池)——管线后置覆盖不变。
- UI:AI 卡槽位 200→216pt;`--snapshot lens` 排版参数;断言 4283→**4323**。

## 14. v3.6.3 变更摘要(2026-09,动态方法论审查:模糊+变异+消毒器)

静态审查(4 路对抗子代理 + 全量第一性原理通读)之后的第三轮:动态方法论——chaos 时间线模糊测试、垃圾 Codable 模糊、蜕变性质、变异测试(30 变异体)、ASan/UBSan 消毒器。断言 2614→**4283**:

- **模糊测试抓到 2 个真 bug**(静态审查三轮都没发现):
  - `statusChangeSummary`:1e308.isFinite==true,有限但超 Int.max 的垃圾 RPM 仍会 Int() 溢出 trap(v3.6.1 的 NaN 防护只挡了 NaN/Inf)——钳 ±9e18。垃圾 Codable 池在确定性种子下稳定复现。
  - **F8** 主路径 DaemonStatus.timestamp 吃默认参数 Date() 绕过 hooks.now() 注入时钟(P7 违规——故障路径显式赋值、主路径漏);生产影响有界但仿真时间轴全失真。
- **变异测试(30 变异体,杀 28+2 补洞后杀)**:净得分 100%。4 个测试盲区补齐:SSD 不变量扫描期望值字面量化(原从实现常量反推=循环论证,P6 变异体靠它存活)、安全阈值整点边界测试(92/78/70/48/45+88 滞回)、试探协议交还时序(C2)、周期重申节奏(C7:[1,20,40] 写入分布)。
- **L1/L2 加固**(模糊前的预验证假设,全部被性质测试锁定):ThermalLearn.percent 读出侧钳位 [0,100](垃圾解码越界值不得外泄);shape/slew NaN 目标 → 0(min/max 的 NaN 穿透会得 100 满速)。
- **ASan+UBSan**:全套件零报告。
- **两起自摆乌龙(诚实记录)**:①变异脚本 git-checkout 还原把未提交的 v3.6.3 修复一并回滚,首轮 campaign 结果作废——教训:变异测试必须基于已提交状态;②修 C7 测试的"风扇跟随"时误删了自己的断言,产生假绿 4282——教训:修测试时不得顺手动断言。均已入 EVOLUTION。
- 新增 TestsFuzz.swift:chaos 时间线×3 种子(600 拍/段,随机事件流下逐拍检查安全红线/值域/收敛性/调度合法性)、垃圾 Codable 池、蜕变性质(单调性/反对称性/幂等性),全部确定性 LCG 种子,CI 零 flake。

## 15. v3.6.2 变更摘要(2026-09,第一性原理发散审查收尾)

全覆盖逐文件发散审查(物理/信息流/时间/失败/权限/并发/语义七轴)的 7 项收尾修复:

- **F2 权限反转**:saveConfig 原子写不补 chown——config.json 损坏后 daemon 自愈重写会把文件变 root:wheel,App 用户从此静默失去写配置能力。root 写入时补 group=admin。
- **F1 loadModel 损坏可观测**:v3.6.1 漏修的最后一个静默加载器(数天辨识参数无提示清零)。
- **F7 备份上限**:损坏备份只保留最新 5 个(毫秒级文件名防同秒覆盖)。
- **F4 确认窗秒基化**:AI 夺回确认(2 拍→5s)与锚定冷却(5 拍→15s)改秒标定——1s 快拍下旧拍数语义缩水 3 倍;dt=3 时与旧行为严格等价(HIL/族扫描全绿佐证)。
- **F5 horizon 卫生**:异常久远(>24h)的 quietUntil/boostUntil 视为无效——手改 config 可把静音封顶/冲刺全速钉死数年;冲刺侧并入过期路径恢复 auto,管线与引擎两条判定同源。
- **F3 推荐采样清场**:AI 目标推荐产出后停止基线采样(原实现 learnedPoints==0 期间无界增长)。
- **F6 看门狗注释更新**。
- **事故与防线(诚实记录)**:v3.6.1 的 tempChange 测试块漏建测试环境,saveConfig 把默认 curve 配置写进真实 config.json,daemon 热加载后用户被切离 AI 模式;当日恢复并修测试。教训:引擎级测试凡触碰 ConfigStore 必须先 engineTestEnv——已写入 EVOLUTION。

断言 2603→**2614**(3× 连绿,族扫描/HIL 与基线一致)。

## 16. v3.6.1 变更摘要(2026-09,对抗式审查稳定版)
...
## 17. v3.6 变更摘要(2026-09,进化传播链 + 数据裁判)
...
## 18. v3.4 变更摘要(2026-09,泛化鲁棒性 + 诚实化 + 可发现性)
...
## 21. v3.1 变更摘要(2026-08,观察期测量 + 循环抑制退避)
...
## 22. v3.0 变更摘要(2026-08,运行态实证 + 稳定版)
...
## 23. v2.9 变更摘要(2026-08,AI 模式/自学习专项)
