...
- 4.2.3：`status.json` 新增 daemon 自报版本（`daemonVersion`），诊断包的"装机"小节从此能
  直接回答"这台机器上跑的是哪版 daemon"——此前只能靠 App plist 版本与二进制 mtime 推断，
  遇到"App 已升级、daemon 留在旧版"的半途状态无从判断。旧 daemon 不写此字段，报告如实标
  "未落盘"。解码侧限长 64 且只收可打印 ASCII（status.json 就在同组可写目录里）。
